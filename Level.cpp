...
namespace LevelSetting {
	...
	constexpr int level_spawn_rates[] = {90, 60, 30};
};
...
Level::update() {
    ...
    monster_spawn_counter = LevelSetting::level_spawn_rates[level - 1]; // Use lvl-1 for 0-based index
...
Level::draw() {
    ...
    //     // al_draw_rectangle(x1, y1, x2, y2, al_map_rgb(100, 100, 100), 2.0);
	// }
	FontCenter *FC = FontCenter::get_instance();
	const char *stage_text = "Stage:";
    int stage_number = level; 

    al_draw_textf(
        FC->courier_new[FontSize::LARGE], 
