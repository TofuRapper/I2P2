...
Monster::Monster(const vector<Point> &path, MonsterType type) {
	...
	switch (type) {
        case MonsterType::WOLF:
            speed = 1 + DC->level->current_level()*2; // ??¼ç?????åº?
            break;
        case MonsterType::CAVEMAN:
            speed = 2 + DC->level->current_level()*2; // æ´?ç©´äºº??????åº?
            break;
        case MonsterType::WOLFKNIGHT:
            speed = 3 + DC->level->current_level()*2; // ??¼é??å£«ç?????åº?
            break;
        case MonsterType::DEMONNIJIA:
            speed = 4 + DC->level->current_level()*2; // ??¡é??å¿??????????åº?
            break;
        default:
            speed = 5 + DC->level->current_level()*2; // ???è¨­é??åº?
    }
}
...
Monster::update() {
	...
	double movement = speed / DC->FPS * 20;
...
void Monster::explosion() {
    ...
    if (explosion_timer > 0) {
        ...
        ImageCenter *IC = ImageCenter::get_instance();
        ALLEGRO_BITMAP *explosion_bitmap = IC->get("./assets/image/explosion.png");

    
        al_draw_bitmap(explosion_bitmap, x - al_get_bitmap_width(explosion_bitmap) / 2, 
                       y - al_get_bitmap_height(explosion_bitmap) / 2, 0);
    }

    // ALLEGRO_SAMPLE *explosion_sound = al_load_sample("./assets/sound/explosion.wav");
    // if (explosion_sound) {
    //     al_play_sample(explosion_sound, 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, nullptr);
    //     al_destroy_sample(explosion_sound); // ?????¾é?³æ??è³?æº?
    // }
}
